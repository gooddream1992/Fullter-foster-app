...
class EditWeightLog extends StatefulWidget {
  @override
  _EditWeightLog createState() => new _EditWeightLog();
}

class _EditWeightLog extends State<EditWeightLog> {
  AppState state;
  String error = '';
  bool status = false;
  ...
  void initState() {
    ...
    if (state.weight['date'] != null) {
      date.text = state.weight['date'];
    }
    if (state.weight['time'] != null) {
      time.text = state.formatTime(state.weight['time']);
    }
    if (state.weight['pre_weight'] != null) {
      preWeight.text = state.weight['pre_weight'];
    }
    if (state.weight['post_weight'] != null) {
      postWeight.text = state.weight['post_weight'];
    }
    if (state.weight['u_m'] != null) {
      if (!units.contains(state.weight['u_m'])) {
        otherUnit = true;
        um = 'others';
      } else {
        otherUnit = false;
  ...
  void submit() {
    ...
    state.postAuth(
        context, 'update-foster-weight/' + state.weight['id'].toString(), {
      'date': date.text,
      'time': time.text,
